...
import {
  ...
  createCaseHearing,
  deleteCitizenCaseDocument,
  deleteCaseHearing,
  downloadCitizenCaseDocument,
  requestLegalAid,
  retryCitizenDocumentOcr,
  scheduleCaseHearingReminder,
  uploadCitizenCaseDocument,
  updateCaseHearing,
  updateCitizenCase
} from "./actions";
...
export default async function CitizenCaseDetailPage({ params }: { params: PageParams }) {
  ...
  const [caseRecord, timeline, hearings, suggestions, legalAidRequests, caseDocuments] = token
    ? await Promise.all([
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="rounded-md border border-border bg-white shadow-panel">
        ...
        <div className="grid gap-6 p-4 lg:grid-cols-[360px_1fr] md:p-5">
          <form action={uploadCitizenCaseDocument.bind(null, caseRecord.id)} className="space-y-4">
            <label className="block">
            ...
            <p className="text-xs leading-5 text-muted-foreground">
              Uploads use short-lived signed storage URLs. Download links are issued only after
...
function CitizenDocumentCard({ caseId, document }: { caseId: string; document: DocumentRecord }) {
  return (
    <div className="rounded-md border border-border p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <StatusBadge>{document.malware_scan_status.replaceAll("_", " ")}</StatusBadge>
        </div>
      </div>
      {document.ocr_text ? (
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <form action={downloadCitizenCaseDocument.bind(null, caseId, document.id)}>
          <button
            className="focus-ring inline-flex h-9 items-center justify-center gap-2 rounded-md border border-border px-3 text-sm font-medium hover:bg-muted"
            type="submit"
          >
        ...
        <form action={retryCitizenDocumentOcr.bind(null, caseId, document.id)}>
          <button
            className="focus-ring inline-flex h-9 items-center justify-center gap-2 rounded-md border border-border px-3 text-sm font-medium hover:bg-muted"
            type="submit"
          >
        ...
        <form action={deleteCitizenCaseDocument.bind(null, caseId, document.id)}>
          <button
